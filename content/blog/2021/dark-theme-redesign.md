...
The last piece of the puzzle is the theme switcher button <i class="ln-dark-mode"></i>/<i class="ln-light-mode"></i> which lets the user set the ``sessionStorage`` ``"dark-mode"`` property, and we've got it all covered. Note that the button must be updated, as it shows a sun in dark mode and a moon in light mode.
...
It contains two functions and an initial setup. The second function, ``darkModeToggle()``, is run whenever the user clicks on the dark mode UI control (the little moon/sun <i class="ln-dark-mode"></i>/<i class="ln-light-mode"></i> to the top-right of the page). It sets the ``"dark-mode"`` property in the ``sessionStorage``. 
