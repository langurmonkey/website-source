...
When I was a kid my parents got me and my brothers a brand new Game Boy. I used to play it a lot, and I was mesmerized and amazed at how such a tiny brick could give life to so many wonderful worlds. Some of my favorite games were **Super Mario Land** (1, 2, and 3, Wario Land), **The Amazing Spider-Man**, and **Kirby's Dream Land**. We also had some other games, like **Bugs Bunny Crazy Castle** or **Tetris**, that I also played quite a lot, and remember fondly.

<!--more-->

The first Super Mario Land in particular was magical to me. I would complete it, and then do it again in one sitting. There were no save games, as it used MBC1 with no RAM (more on that later), so you really had to *git gud*. The second play through had the same levels but more enemies, in what must have been one of the first implementations of *New Game+*. After this harder mode was completed, you could select and play any level. Too bad all this went away as soon as you powered off the device. **Super Mario Land 2: 6 Golden Coins** already introduced save games (used MBC1+RAM+Battery), and it was a huge step-up in QoL. In any case, this little device has always meant much to me.
...
Some time ago, I wrote a [CHIP-8 emulator](/blog/2021/chip8-implementation/) in Rust, and found the experience very gratifying. The consensus online seems to be that the next step is the Game Boy (DMG, Dot Matrix Game). I know that the internet is flooded with "Implementing a Game Boy emulator" kind of articles, and (judging by the GitHub counts) it is a very unoriginal project, but I still wanted to try my hand at it. So I took the bait and started working on a Game Boy emulator. Today, I'm excited to share my very own Game Boy emulator written in Rust and SDL2.

The original Game Boy, released in 1989, is an excellent target for emulation. Its hardware is rather well-documented, its architecture is relatively simple compared to modern systems, yet it's complex enough to present real challenges, which I for sure faced.

I called my implementation [**Play Kid**](/projects/playkid). It is written in Rust and uses SDL2 for graphics, sound, and input. Here are the main features:
...
- [Instruction table](https://meganesu.github.io/generate-gb-opcodes/) -- all 8-bit opcodes into a single table, complete with descriptions for each one of them. Very handy. 
...
The Game Boy's execution model is pretty simple. The CPU has a 16-bit program counter register (PC) that points to the next instruction in memory. Each cycle, it fetches the byte at that address, decodes it into an instruction, executes it, and increments the **PC**. Instructions are composed of opcode+operand, and can be one, two, or three bytes long. Each instruction takes a specific number of machine cycles (**M-cycles**) to complete, usually between 1 and 6. Since the CPU runs at around 4.19 MHz and one M-cycle equals four clock cycles (**T-cycles**), timing is important.
...
Interrupts let hardware grab the CPU's attention. When something happens (the display finishing a frame, or a timer overflow), the corresponding interrupt flag is set. If interrupts are enabled (via the IME flag), the CPU pauses what it's doing, pushes the current PC onto the stack, and jumps to a predefined interrupt handler address. This lets games sync with hardware events instead of constantly checking for them.

The hardware components all move forward together based on the CPU's cycle count. When the CPU executes a 4 M-cycle instruction, the PPU advances 16 T-cycles worth of scanline rendering, the APU moves its sound generation forward, and timers tick along. Everything runs in lockstep, which means accurate emulation needs cycle-level precision across all the different parts.
...
The CPU features eight 8-bit registers (A, B, C, D, E, F, H, L) that can be paired into four 16-bit registers (AF, BC, DE, HL), plus a 16-bit stack pointer (SP) and the abovementioned program counter (PC). The F register is special, as its four most significant bits hold flags that track the results of operations:
...
Actually implementing the CPU meant building a `match` with cases for ever single one of the 512 instructions. Each instruction updates the PC, manipulates registers or memory, and sets flags according to specific rules. Some instructions were very straightforward to implement; others, like <kbd>DAA</kbd> (Decimal Adjust Accumulator), required careful consideration of edge cases and bit manipulation. The implementation structure is quite straightforward:
...
The CPU doesn't run in isolation—it needs to be cycle-accurate. Each instruction takes a specific number of machine cycles (M-cycles), typically 1-6, and the rest of the system needs to advance accordingly. Getting timing right is crucial because the graphics hardware, audio, and even some games depend on precise cycle counts. This is why `execute(...) -> u8` returns an unsigned integer. It is the number of cycles that the instruction takes. The main loop handles the input events, and then runs as many cycles per frame as necessary.
...
6. Update the IME (Interrupt Master Enable) and handle interrupts
...
We'll talk about the memory cycle when we reach the memory section. For now, let's look at interrupts, as it was an interesting challenge. The Game Boy supports five interrupt types: V-Blank, LCD STAT, Timer, Serial, and Joypad. The CPU checks for pending interrupts after each instruction, and if interrupts are enabled and one is pending, it pushes the PC onto the stack and jumps to the corresponding interrupt vector. Implementing the Interrupt Master Enable (IME) flag and its one-instruction delay after the <kbd>EI</kbd> instruction was a subtle detail that broke several games until I got it right. This is handled in `ime_update()` and `interrupt_handling()`:
...
The memory struct contains the Work RAM, the High RAM, IO registers, the IF and IE, a reference to the PPU (seen later in this post), and the APU (also later), a reference
to the cartridge (seen later as well), the timer, and the joypad.
...
```
Memory-mapped I/O registers control everything from LCD modes to audio channels. Some registers are read-only, some write-only, and some have bits that behave differently on read versus write. The DMA (Direct Memory Access) register at 0xFF46 is particularly interesting. Writing to it triggers a transfer of 160 bytes from ROM or RAM to OAM, during which the CPU should only access High RAM. Getting DMA timing correct was essential for sprite-heavy games.

## The PPU

The Picture Processing Unit is what puts pixels on the screen. It generates a 160x144 display running at around 60 Hz. Getting it working meant understanding scanlines, timing, and how backgrounds, windows, and sprites all come together.
...
Sprites (or objects) are 8x8 or 8x16 pixels that sit on top of the background. The OAM (Object Attribute Memory) holds up to 40 sprite definitions, each with position, tile index, and attributes like palette selection, flip flags, and priority. The hardware can only show 10 sprites per scanline, and when more than 10 overlap, priority goes to sprites with lower X coordinates first, then lower OAM index:
...
Getting the timing right was kind of important because some games rely on precise V-Blank and H-Blank timing to avoid visual glitches.

In order to test my PPU implementation, I turned to the [DMG Acid2](https://github.com/mattcurrie/dmg-acid2) test. This is a rather exhaustive PPU test in the form of a ROM. If you see a happy face with a couple of texts above and below, you're good. In my case, the right chin was missing. I spent an ginormous amount of time trying to get it to render properly. This is what I was getting:
...
Now that my PPU was in shape, I tested it with other games. Seeing the first sprite render correctly, or watching the background scroll smoothly for the first time, made all the debugging worth it (not you, right chin!). The PPU is the most visible part of the emulator, so getting it right feels really satisfying.

At the very end, when my emulator was already in a very good shape, I implemented multiple color palettes. The original Game Boy was monochrome, but you can map those four gray shades to any colors you want. I included a few classic palettes (DMG Classic, Pocket, etc.) and some more creative ones. You can cycle through them while playing with <kbd>p</kbd>, which makes the games look fresh.
...
I consider the Audio Processing Unit (APU) to be one of the most boring parts of this project. The APU synthesizes sound through four channels: two pulse wave channels (square waves), one wave channel (custom waveform), and one noise channel (pseudo-random). Each channel has its own registers for frequency, volume, envelope, and duty cycle.

The APU has a frame sequencer that runs at 512 Hz, stepping through eight different phases. Some phases update length counters, some update the sweep function, and some update volume envelopes. My implementation tracks this with a frame timer that increments with each T-cycle:
```rust
...
The noise channel (channel 4) uses a Linear Feedback Shift Register (LFSR) to generate pseudo-random noise. The LFSR is a 15-bit shift register that XORs bits 0 and 1, then shifts the result back in:
```rust
...
Getting the timing and overflow behavior right required reading Pan Docs extensively and testing with games that use sweep heavily.
...
Interestingly, in Super Mario Land, the music is mostly in channels 2 and 3. Channel 1 is for things like jumps, coins, and some high-pitched parts of melodies. Channel 4 is used in the random noise that some enemies do when they die (like the bomb turtles), and some percussion beats in the background.
...
Most Game Boy cartridges aren't just ROM chips. They contain memory bank controllers (MBCs) that expand the addressable memory beyond the CPU's 64 KB limit. Different games use different MBCs, each with its own banking scheme.

Every cartridge has a header header that contains essential information about the game, stored in the first few bytes of the cartridge's ROM. This information helps the system identify and load the game properly. Key fields in the header include:
...
Implementing battery-backed SRAM meant saving the contents of cartridge RAM to `.sav` files when the emulator exits and loading them on startup. I get the RAM from the cartridge, and then persist it if it is not empty:
...
This feature is important for games like Pokémon and Super Mario Land 2, where your save file persists between sessions. The save/load mechanism checks the cartridge type and only persists data for mappers that actually have RAM. The `.sav` file is written to the same directory where the ROM file is and with the same name and `sav` extension.
...
Rust was an excellent choice for this project. Its type system caught countless bugs at compile time, and the borrow checker forced me to think carefully about ownership and lifetimes. SDL2 provided a simple, cross-platform foundation for rendering and audio without unnecessary complexity.

If you're interested in emulation, I highly recommend starting with the Game Boy. The documentation is excellent (shout out to Pan Docs and the gbdev community), the hardware is approachable, and seeing a classic game run on your own emulator is incredibly satisfying.

Play Kid is [open source on GitHub](https://github.com/langurmonkey/playkid). Feel free to explore the code, open issues, or submit improvements. There's still work to do—better accuracy, more mappers, potential Game Boy Color support—but for now, I'm happy to have a working emulator that plays my favorite childhood games.

Now, if you'll excuse me, I have a Pokémon journey to continue.
